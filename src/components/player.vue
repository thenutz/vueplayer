	...
	>
		...
		>
			...
			>
				<!-- <video
          class="oxiplay-video"
          id="oxiplay-video-player-jBP"
          videoid="player-jBP"
          crossorigin="true"
          webkit-playsinline="true"
          playsinline="true"
          preload="none"
          poster="http://videowow.ro/image/video_thumb_custom/jBP_430014f4ea356c6fa439235fe530fbb4.jpg"
        >
          <source
            type="video/mp4"
            src="http://videowow.ro/uploads/videos/jBP/jBP.mp4"
          />
				</video>-->
			</div>
			<div
          ...
          loaded:
			...
			>
				<div
					...
					v-if="playerStatus.settings.status"
				>
					<div class="settings_wrapper">
						...
						<div class="ovf">
							...
							>
								<li
									class="lista"
									@click.stop="settingsSlide('quality')"
								>
									<div
										class="cnt"
										v-html="text.quality"
								...
								>
									...
									>
										...
										>
											...
											<div
												class="icon"
												v-if="playerStatus.speed.current === s"
											>
												<Checked />
											</div>
										</li>
				...
				<div class="controls_container">
                    ...
                    'background-position':
                      this.seekbar.filmstripPosition + 'px 0'
								...
								<div
									class="seek-bar"
									:class="{ nt: this.seekbar.isDraging }"
								>
									<div
										class="buffer-bar"
										:id="'buffer_' + videoId"
...
export default {
	...
	mounted() {
		...
		setTimeout(() => {
			let videoObject = {
				type: false,
				sourceUrl: "//thenutz.ro/samples/sample.mp4",
				posterUrl:
					"//thenutz.ro/samples/93d1b16a81c9180a637b4dc6eade947d-xlarge.jpg",
				filmstripUrl: "//thenutz.ro/samples/film_sprite.png",
				duration: 0
			};
			...
			if (canPlay_MP4) {
				...
				videoElement.id = "oxiplay-video-" + this.videoId;
				...
				videoElement.setAttribute("preload", "metadata"); // auto|metadata|none
	...
	methods: {
		...
		setVolume(mute = false) {
			if (mute === "mute") {
				if (this.playerStatus.volume.status === 0) {
					this.playerStatus.videoElement.muted = false;
					this.playerStatus.videoElement.volume =
						(1 * this.playerStatus.volume.previous) / 100;
					this.playerStatus.volume.status = this.playerStatus.volume.previous;
				} else {
					this.playerStatus.videoElement.muted = true;
					this.playerStatus.videoElement.volume = 0;
					this.playerStatus.volume.status = 0;
				}
			} else {
				...
				if (this.playerStatus.videoElement.muted) {
					this.playerStatus.videoElement.muted = false;
				}
			}
		},
		...
		seekDown(location) {
			if (location === "seekbar") {
				this.playerStatus.videoElement.pause();
		...
		checkSeekbarDrag() {
			...
			if (perc < 1) {
				...
				this.playerStatus.videoElement.currentTime = 0;
			} else if (perc > 99) {
				this.playerStatus.videoElement.currentTime = this.videoData.duration;
				this.seekbar.seekPosition = 100;
			} else {
				this.seekbar.seekPosition = perc;
				this.playerStatus.videoElement.currentTime =
					(this.videoData.duration * perc) / 100;
			}
		},
		checkVolumeDrag() {
			...
			if (perc < 1) {
				...
				this.playerStatus.videoElement.volume = 1;
				...
				this.playerStatus.volume.status = Math.round(perc);
				this.playerStatus.volume.previous = Math.round(perc);
				this.playerStatus.videoElement.volume = (1 * perc) / 100;
			}
		},
		seekControl() {
			let x = event.pageX; //Y click position on entire page
		...
		mouseIsOver() {
			event.target.addEventListener("mousemove", this.checkMouseOver, false);
		},
		checkMouseOver() {
			if (!this.playerEvents.isFadeIn) {
				if (this.playerEvents.overflowtimer) {
					clearTimeout(this.playerEvents.overflowtimer);
					this.playerEvents.overflowtimer = 0;
				}
			} else {
				this.playerEvents.isFadeIn = false;
			}
			this.playerEvents.overflowtimer = setTimeout(() => {
				this.playerEvents.isFadeIn = true;
			}, 2000);
		},
		overlayClick() {
			console.log(this.playerStatus.videoElement.paused);
			if (this.playerStatus.status === "loaded") {
				this.playerStatus.videoElement.play();
				this.playerStatus.status = "playing";
			} else if (!this.playerStatus.videoElement.paused) {
				this.playerStatus.videoElement.pause();
